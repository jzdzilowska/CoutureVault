...
export class BackendNodeGateway {
  ...
  /**
   * Method to search nodes by title and content.
   *
   * @param searchTerm - The term to search for in title and content fields.
   * @return IServiceResponse<INode[]> on success
   *         failureServiceResponse on failure
   */
  async search(searchTerm: string): Promise<IServiceResponse<INode[]>> {
    try {
      const searchResponse = await this.nodeCollectionConnection.searchNode(
        searchTerm
      );
      return searchResponse;
    } catch (error) {
      console.error(error);
      return failureServiceResponse("Internal server error.");
    }
  }
}
