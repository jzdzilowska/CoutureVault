...
export const FrontendNodeGateway = {
  ...
  /**
   * This method fetches the backend data for the frontend components to use in displaying
   * searchResuts data.
  ...
  getSearchResults: async (
    ...
    try {
      // Make a request to the backend endpoint for searching nodes
      return get<IServiceResponse<INode[]>>(
        baseEndpoint + servicePath + `search?term=${searchTerm}`
        // `${baseEndpoint}${servicePath}search?term=${searchTerm}`
      );
    } catch (exception) {
      return failureServiceResponse(
        "[getSearchResults] Unable to access backend"
      );
    }
  },
