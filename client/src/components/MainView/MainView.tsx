...
export const MainView = React.memo(function MainView() {
  ...
  const handleHomeClick = useCallback(() => {
    setSelectedNode(null);
    setShowSearchResults(false); // Add this line to hide search results when going back to the main page
  }, []);

  // const getEdges = (currentNode: INode, childNodes: INode[]): ILink[] => {
  //   // Logic to generate edges based on currentNode and childNodes
  //   const edges: ILink[] = [];

  //   // For example, you might iterate through childNodes and create edges:
  //   childNodes.forEach((childNode) => {
  //     edges.push({
  //       anchor1Id: currentNode.nodeId,
  //       anchor2Id: childNode.nodeId,
  //       anchor1NodeId: currentNode.nodeId,
  //       anchor2NodeId: childNode.nodeId,
  //       dateCreated: new Date(),
  //       explainer: "", // You can set explainer based on your logic
  //       linkId: generateObjectId("link"),
  //       title: `Link from ${currentNode.title} to ${childNode.title}`,
  //     });
  //   });
  ...
  //   if (currentNode) {
  //     // Only generate edges if currentNode is not null
  //     childNodes.forEach((childNode) => {
  //       edges.push({
  //         anchor1Id: currentNode.nodeId,
  //         anchor2Id: childNode.nodeId,
  ...
  /**
   * This function connects the frontend and backend, it gets the data that is handled by the searching functionality
   * and is used to communciate this to the frontend in order to display search results.
   * @param searchTerm the word you are entering into the searchBar
   */
  const handleSearch = async (searchTerm: string) => {
    try {
      const response = await FrontendNodeGateway.getSearchResults(searchTerm);
      if (!response.success) {
        ...
        <div className="main-container">
          ...
          <div className="content">
            ...
            <div className="node-wrapper">
              {/* Searchbar is rendered here */}
              <SearchBar onSearch={handleSearch} />

              {showSearchResults ? (
                <SearchView
                  searchResults={searchResults} // sends the payload data to searchView for searchResults rendering
                />
              ) : (
                <NodeView
                  childNodes={
